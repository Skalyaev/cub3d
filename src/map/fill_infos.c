...
int get_colors(char *str, int *i, int l, char ref)
{
        ...
        while (str[*i] && str[*i] != '\n' && str[++(*i)])
        {
                if (!ft_isdigit(str[*i]) && !ft_ischarset(" ,", str[*i]) && ft_putstr_fd("Error\nInvalid colors\n", 2))
                        return (0);
                if (ft_isdigit(str[*i]))
                {
                        k = 0;
...
{
        if (str[*i] && str[*i + 1] && str[*i] == 'F' && str[*i + 1] == ' ' && ++(*i) && !get_colors(str, i, -1, 'f'))
                return (0);
        if (str[*i] && str[*i + 1] && str[*i] == 'C' && str[*i + 1] == ' ' && ++(*i) && !get_colors(str, i, -1, 'c'))
                return (0);
...
int fill_map_struct(char *str, int i)
{
        while (str[i])
        {
                if (str[i + 1] && str[i + 2] && str[i] == 'N' && str[i + 1] == 'O' && str[i + 2] == ' ' && !texture_path_finded(str, &i, 'n'))
                        return (0);
                if (str[i] && str[i + 1] && str[i + 2] && str[i] == 'S' && str[i + 1] == 'O' && str[i + 2] == ' ' && !texture_path_finded(str, &i, 's'))
                        return (0);
                if (str[i] && str[i + 1] && str[i + 2] && str[i] == 'E' && str[i + 1] == 'A' && str[i + 2] == ' ' && !texture_path_finded(str, &i, 'e'))
                        return (0);
                if (str[i] && str[i + 1] && str[i + 2] && str[i] == 'W' && str[i + 1] == 'E' && str[i + 2] == ' ' && !texture_path_finded(str, &i, 'w'))
                        return (0);
