...
{
        while (j - ++k >= 0 && (data())->map.map[i][j - k] != 0)
                if ((data())->map.map[i][j - k] == 2 && ++count)
                        break;
        k = 0;
        while ((data())->map.map[i][j + ++k] != -1 && (data())->map.map[i][j + k] != 0)
                if ((data())->map.map[i][j + k] == 2 && ++count)
                        break;
        return ((count == 2));
}
...
int check_map_walls(int i, int j)
{
        while ((data())->map.map[++i])
        {
                ...
                {
                        if ((data())->map.map[i][j] == 1 || (data())->map.map[i][j] == 4 || (data())->map.map[i][j] == 5 || (data())->map.map[i][j] == 6 || (data())->map.map[i][j] == 8)
                        {
        ...
        while ((data())->map.map[++i])
        {
                ...
                while ((data())->map.map[i][++j] != -1)
                {
                        ...
                        if ((data())->map.map[i][j] == 2 && !finded[1] && ++count)
                                finded[1] = 1;
                        if (((data())->map.map[i][j] == 8 || (data())->map.map[i][j] == 6 || (data())->map.map[i][j] == 4 || (data())->map.map[i][j] == 5) && ++count)
                                finded[2] = 1;
                }
        }
...
int check_map_infos(void)
{
        ...
        if (e_count != 2 && e_count != 3 && ft_putstr_fd("Error\nWrong map syntax\n", 2))
                return (0);
        if (!check_map_walls(-1, -1) && ft_putstr_fd("Error\nWalls don't cover the map\n", 2))
                return (0);
        return (1);
}
