...
void put_from_tex(int x, int y, int i, int j)
{
        ...
        j = (y - (data())->draw_start - (data())->tex_y_offset) * (float)((float)(data())->actual_tex->height / (data())->line_size);
        if ((data())->side == 0 && (data())->ray_dir_x > 0)
                i = (data())->actual_tex->width - i - 1;
        if ((data())->side == 1 && (data())->ray_dir_y < 0)
                i = (data())->actual_tex->width - i - 1;
        ft_put_pixel_to_image(&((data())->display), x, y, *(unsigned int *)((data())->actual_tex->addr + ((j * (data())->actual_tex->line_length) + (i * ((data())->actual_tex->bits_per_pixel / 8)))));
}
...
int draw_line(int x, int y, int color)
{
        ...
        {
                if (y < (data())->draw_start || y > (data())->draw_end || (data())->dont_draw)
                {
